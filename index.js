...
admin.initializeApp({
  credential: admin.credential.cert({
    ...
    client_email: process.env.CLIENTEMAIL, private_key: process.env.PRIVATEKEY.replace(/\\n/g, "\n")
  }),
...
function postLevel(theLevelName, levelData, pw, color, gravity, side, socket) {//communiates stuff to firebase, this is mostly very hard to read, don't bother if you don't want to
  ...
  hello = false;
  count = 0;
  if (theLevelName == "") { socket.emit("empty_name") }
  else {
    db
      ...
      .then(() => {
        if (!hello) {
          db.collection("metadata").doc("documentString").get().then(doc=>{
            ...
            db.collection("metadata").doc("documentString").update({list:JSON.stringify(hello)});
          });
      ...
      snapshot.forEach(doc => {
        ...
        if (!done && dataName == theLevelName) {
          ...
          if (editor) {
            ...
            if(doOld=="false"){
              socket.emit("level_data",doc.data().data,{color:pc,jumpHeight:grav,sideSpeed:side})
            }
            else {socket.emit('level_data', doc.data().data, {color:pc, jumpHeight:grav, sideSpeed:side});}
          } else {
            res.render((doOld == "true" ?"":"newUI") + 'level', {
              linkToEdit: "/editor/" + encodeURIComponent(dataName).replace(/\./g,"%2E"),
              title: "User Level " + dataName,
              onloadThing: sideString + colorString + gravString + 'setupWithArray("' + doc.data().data + '");' + (ThreeD == "true" ? "doVels=null;renderer.render(scene,camera);" : ""), isRandom: "__NO__",
...
io.sockets.on('connection', socket => {
  ...
  socket.on('listLevels',()=>{
    db.collection("metadata").doc("documentString").get().then(doc=>{
      socket.emit("levelList",JSON.parse(doc.data().list).map(i=>{return decodeURIComponent(i).replace("%2E",".")}));
    })
  })
...
app.use((req,res,callback)=>{
  ...
  res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");callback();})
...
app.get('/test3d', (req, res) => {
  ...
  res.render("level", {
    ...
    linkToEdit: "/editor/random/" + randomNumThing,
    title: "Blocky Jump: Make, play, and share platforming levels!", isRandom: randomNumThing, prevLink: "/random/" + randomNumThing - 1, nextLink: "/random/" + randomNumThing + 1, doThreeD: "TRUE"
  });
});
